<?php
/*
* Template Name: Chart Firing Arcs
*/
 get_header(); ?>
<?php if ( have_posts() ) while ( have_posts() ) : the_post(); ?>
	...
	<column class="col-4 col-chart-firing-arcs col-chart-action">
        <div class="filterbar">   
             <factionbar>
                <?php 
                    $cnt = 0; 
    	    ...
    	    <arcs class="action-select-bar">
				<?php 
                    $cnt = 0; 
                    $terms_type = get_terms('firing-arcs-tax');
                    foreach ( $terms_type as $term_type ) :
                    if(++$cnt > 1) { echo '<span class="splitter"></span>'; } 
                ?>
                    <action actionselect="<?php echo $term_type->slug; ?>" showupgrade="show">
                        <arc class="<?php echo $term_type->slug; ?>"></acr>
                    </action>
                                        ...
                                        <div class="chart-items">
                                            <arcs>
                                                <?php $cnt = 0; 
                                                    ...
                                                    <action>
                                                        <?php 
                                                            $hasaction = 0; $attack = 0;
                                                            $arcterms = get_field('firing_arc', $conf->ID);
                                                            foreach( $arcterms as $arcterm ): 
                                                                if($term_type->slug == $arcterm->slug) {
                                                                    $hasaction = 1;
                                                                    $attack = get_field('primary_weapon_value', $conf->ID);
                                                                }
                                                            endforeach;
                                                            $arcterms = get_field('secondary_firing_arc', $conf->ID);
                                                            foreach( $arcterms as $arcterm ): 
                                                                if($term_type->slug == $arcterm->slug) {
                                                                    $hasaction = 1;
                                                                    $attack = get_field('secondary_weapon_value', $conf->ID);
                                                                }
                                                            endforeach;
                                                            if($hasaction == 1) {
                                                                echo '<actioncon actiontype="'.$term_type->slug.'">';
                                                                    echo '<arc class="'.$term_type->slug.'" showupgrade=""></arc>';
                                                                    echo '<div>';
                                                                    for($x = 1; $x <= $attack; $x++) {
                                                                        echo '<div class="attack-dot"></div>';
                                                                    }
                                                                    echo '</div>';
                                                                echo '</actioncon>';
                                                            }
